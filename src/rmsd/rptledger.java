...
public class rptledger extends javax.swing.JInternalFrame {
    ...
    void cteateItemOpenBalance(String fromdate) {
        String sql = "";
        try {
            sql = "delete FROM `rmsd_db`.`temp_ledger_opbal`";
            dbconnector.insertquery(sql);
       //    sql = "create table temp_ledger_opbal SELECT t.`g_itemcode`,sum(t.`g_Rqnty`- t.`g_Iqnty`) as balance, sum(t.`g_qnty`) as bal FROM `transaction` t where t.`g_date` < '" + fromdate + "'  group by t.`g_itemcode`;";
            sql = "insert into temp_ledger_opbal select  t.`g_itemcode` as g_itemcode,sum(t.`g_Rqnty`- t.`g_Iqnty`) as balance, sum(t.`g_qnty`) as bal FROM `transaction` t where t.`g_date` < '" + fromdate + "'  group by t.`g_itemcode`";
            dbconnector.insertquery(sql);
